...
def fetch_all_card_data
  ...
  page = 1
  loop do
    response = HTTParty.get("https://api.scryfall.com/cards/search?q=format%3Apauper&page=#{page}")
...
if cards_data.present?
  cards_data.each do |card_data|
    if card_data['legalities'].present? && card_data['legalities']['pauper'] == 'legal'
